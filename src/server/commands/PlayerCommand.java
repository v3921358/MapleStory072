...
//            sb.append("\r\n@CGM <讯息>  -- 传送讯息給GM");
    ...
    public static class 爆XXL extends Mobdrop {
    }
    ...
    public static class 查看 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "欢迎来到072冒险岛,祝你游戏愉快!（经验值 " + (Math.round(c.getPlayer().getEXPMod()) * 1) * Math.round(c.getPlayer().getStat().expBuff / 100.0) + "倍）, （怪物爆率 " + (Math.round(c.getPlayer().getDropMod()) * 1) * Math.round(c.getPlayer().getStat().dropBuff / 100.0) + "倍）, （金币爆率 " + Math.round(c.getPlayer().getStat().mesoBuff / 100.0) * 1 + "倍）");
