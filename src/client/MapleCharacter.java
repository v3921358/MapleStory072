    ...
    public static MapleCharacter ReconstructChr(final CharacterTransfer ct, final MapleClient client, final boolean isChannel) {
        ...
        if (isChannel) {
            ...
            if (ret.map == null) { //char is on a map that doesn't exist warp it to spinel forest
                ...
                if (ret.map.getForcedReturnId() != 999999999 && ret.map.getForcedReturnMap() != null && ret.mapid != GameConstants.JAIL) {
...
//            }
                ...
                /*
                     * ps = con.prepareStatement("SELECT * FROM characters WHERE
                     * accountid = ? ORDER BY level DESC"); ps.setInt(1,
                     * ret.accountid); rs = ps.executeQuery(); int maxlevel_ = 0,
                     * maxlevel_2 = 0; while (rs.next()) { if (rs.getInt("id") !=
                     * charid) { // Not this character if
                     * (GameConstants.isKOC(rs.getShort("job"))) { int maxlevel =
                     * (rs.getShort("level") / 5);
                     *
                     * if (maxlevel > 24) { maxlevel = 24; } if (maxlevel >
                     * maxlevel_2 || maxlevel_2 == 0) { maxlevel_2 = maxlevel;
                     * ret.BlessOfEmpress_Origin = rs.getString("name"); } } int
                     * maxlevel = (rs.getShort("level") / 10);
                     *
                     * if (maxlevel > 20) { maxlevel = 20; } if (maxlevel >
                     * maxlevel_ || maxlevel_ == 0) { maxlevel_ = maxlevel;
                     ...
                     * new SkillEntry(maxlevel_, (byte) 0, -1)); if
                     * (SkillFactory.getSkill(GameConstants.getEmpress_ForJob(ret.job))
                     * != null) { if (ret.BlessOfEmpress_Origin == null) {
                     ...
                     * new SkillEntry(maxlevel_2, (byte) 0, -1)); } ps.close();
                     * rs.close();
                     */
                    // END
        ...
        while (!passed) {
            ...
            switch (randval) {
                case 0: // Meso
                    ...
                    break;
                case 1: // EXP
                    final int experi = Randomizer.nextInt(Math.min((Math.abs(getNeededExp() / 200) + 1), 500000));
...
//        System.out.println(start + " | " + end);
    ...
    public void gainExpMonster(final int gain, final boolean show, final boolean white, final byte pty, int Class_Bonus_EXP, int Equipment_Bonus_EXP, int Premium_Bonus_EXP, boolean partyBonusMob, final int partyBonusRate) {
        ...
        int total = gain + Class_Bonus_EXP + Equipment_Bonus_EXP + Premium_Bonus_EXP + Marriage_Bonus_EXP + partyinc + Activity_Bonus_EXP;
        ...
        if (gain != 0) {
            ...
            if (show) { // still show the expgain even if it's not there
                client.getSession().write(InfoPacket.GainEXP_Monster(expM, white, partyinc, Class_Bonus_EXP, Equipment_Bonus_EXP, Premium_Bonus_EXP, Marriage_Bonus_EXP, Team_Bonus_Rate, Activity_Bonus_EXP));
            }
        }
    }
                ...
                if (pet != null && (item.getItemId() != 5000054 || pet.getSecondsLeft() > 0) && (item.getExpiration() == -1 || item.getExpiration() > System.currentTimeMillis())) {
                    if (pet.getSummoned()) { // Already summoned, let's keep it
                        ...
                        if (/*broadcast &&*/getMap() != null) {
                            //client.getSession().write(PetPacket.updatePet(pet, getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), pet.getSummoned()));
